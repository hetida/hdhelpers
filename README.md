...
6) To check if hdhelpers is still compatible with the newest version of hetida designer run
`uv pip sync requirements.txt requirements-dev.txt requirements-base.txt requirements-local-dev.txt`
...
Before you build the package, set an appropriate version number in `pyproject.toml` that matches the version number in the hetida designer `VERSION` file.
